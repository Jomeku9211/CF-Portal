...
class OrganizationService {
  ...
  async createOrganization(data: CreateOrganizationData): Promise<OrganizationResponse> {
    try {
      console.log('Sending request to:', getSupabaseUrl(SUPABASE_ENDPOINTS.ORGANIZATIONS));
      console.log('Request headers:', this.getAuthHeaders());
      console.log('Request body:', JSON.stringify(data, null, 2));
      
      const response = await fetch(`getSupabaseUrl(SUPABASE_ENDPOINTS.ORGANIZATIONS)`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        if (looksLikeIndustryEnumError) {
          ...
          const retryRes = await fetch(`getSupabaseUrl(SUPABASE_ENDPOINTS.ORGANIZATIONS)`, {
            method: 'POST',
            headers: this.getAuthHeaders(),
  ...
  async getOrganizationById(id: string): Promise<OrganizationResponse> {
    try {
      const response = await fetch(`getSupabaseUrl(SUPABASE_ENDPOINTS.ORGANIZATIONS)/${id}`, {
        method: 'GET',
        headers: this.getAuthHeaders(),
      });
