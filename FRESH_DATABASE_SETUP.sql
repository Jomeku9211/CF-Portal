-- =====================================================
-- FRESH DATABASE SETUP - WIPE ALL AND CREATE NEW
-- =====================================================
-- This script will completely reset your database and create the new flexible structure
-- WARNING: This will delete ALL existing data!

-- =====================================================
-- STEP 1: COMPLETELY WIPE EXISTING DATABASE
-- =====================================================
...
-- =====================================================
-- STEP 2: CREATE NEW FLEXIBLE DATABASE STRUCTURE
-- =====================================================
...
CREATE POLICY "Users can view applications for their projects." ON public.project_applications FOR SELECT USING ((SELECT auth.uid()) IN (SELECT client_id FROM public.projects WHERE id = project_id));
...
-- =====================================================
-- 16. CREATE FUNCTION TO ENSURE ONLY ONE PRIMARY ROLE PER USER
-- =====================================================

-- Function to check and update primary role
CREATE OR REPLACE FUNCTION public.ensure_single_primary_role()
RETURNS TRIGGER
AS $$
BEGIN
  -- If this role is being set as primary, unset all other primary roles for this user
...
CREATE TRIGGER ensure_single_primary_role_trigger
  ...
  FOR EACH ROW EXECUTE PROCEDURE public.ensure_single_primary_role();
...
-- Set up access controls for storage (with IF NOT EXISTS checks)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Anyone can upload an avatar.' AND tablename = 'objects') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Portfolio images are publicly accessible.' AND tablename = 'objects') THEN
        CREATE POLICY "Portfolio images are publicly accessible." ON storage.objects FOR SELECT USING (bucket_id = 'portfolio');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can upload portfolio images.' AND tablename = 'objects') THEN
        CREATE POLICY "Users can upload portfolio images." ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'portfolio');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update their own portfolio images.' AND tablename = 'objects') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can delete their own portfolio images.' AND tablename = 'objects') THEN
...
CREATE INDEX idx_service_provider_profiles_user_id ON public.service_provider_profiles(user_id);
...
-- =====================================================
-- 20. INSERT SAMPLE DATA - ROLE CATEGORIES
-- =====================================================
...
-- =====================================================
-- 21. INSERT SAMPLE DATA - EXPERIENCE LEVELS
-- =====================================================
...
-- =====================================================
-- 23. TEST THE FLEXIBLE ROLE SYSTEM
-- =====================================================
-- This demonstrates how users can have multiple roles

-- Example: Show how a user could be both an agency and a client
SELECT 
  'Multi-Role User Example' as example,
  'Agency can also be Client' as scenario,
  'User can hire talent while offering agency services' as explanation;

-- Example: Show how a service provider can also be a client
SELECT 
  'Multi-Role User Example' as example,
  'Service Provider can also be Client' as scenario,
  'Developer can hire other developers for their projects' as explanation;

-- =====================================================
-- SETUP COMPLETE! 🎉
-- =====================================================
SELECT '🎉 Database setup completed successfully!' as status;
SELECT 'Your flexible multi-role system is ready!' as message;
SELECT 'Users can now have multiple roles: client, service-provider, agency' as feature;
