...
export function AboutUsSection() {
  ...
  const whoWeHelpData = {
    founders: {
      ...
      description: 'Every hire is critical, and misfits are too costly.'
    },
    hr: {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {solutionData.map((item, index) => <div key={index} className="bg-blue-50 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="mb-20">
          ...
          <div className="bg-gray-50 rounded-2xl p-6">
            <div className="flex flex-wrap justify-center mb-6">
              {Object.keys(whoWeHelpData).map(key => <button key={key} className={`px-4 py-2 mx-2 mb-2 rounded-md transition ${activeTab === key ? 'bg-blue-700 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`} onClick={() => setActiveTab(key)}>
                  {whoWeHelpData[key].title}
                </button>)}
            </div>
            <div className="bg-white rounded-xl p-6 shadow-sm">
              <div className="flex items-center mb-4">
                {whoWeHelpData[activeTab].icon}
                <h4 className="text-xl font-semibold text-gray-900 ml-3">
                  {whoWeHelpData[activeTab].title}
                </h4>
              </div>
              <p className="text-lg text-gray-700">
                {whoWeHelpData[activeTab].description}
              </p>
            </div>
          </div>
        </div>
